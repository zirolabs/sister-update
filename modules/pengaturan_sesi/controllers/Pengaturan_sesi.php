...
class Pengaturan_sesi extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('pengaturan_sesi_model');
		$this->page_active 		= 'pengaturan';
		$this->sub_page_active 	= 'pengaturan_sesi';
	}
	...
	public function index()
	{
		...
		$param['data']			= $this->pengaturan_sesi_model->get_data($filter)->result();

		unset($filter['limit']);
		unset($filter['offset']);
		$total_rows 			= $this->pengaturan_sesi_model->get_data($filter)->num_rows();
		$param['pagination']	= paging('pengaturan_sesi/index', $total_rows, $limit, $uri_segment);

		$param['main_content']		= 'pengaturan_sesi/table';
	...
	public function form($id = '')
	{
		...
		else
		{
			if(!empty($id))
			{
				$param['data'] = $this->pengaturan_sesi_model->get_data_row($id);
			}
		}

		$param['main_content']		= 'pengaturan_sesi/form';
		$param['page_active'] 		= $this->page_active;
		$param['sub_page_active'] 	= $this->sub_page_active;
		$this->templates->load('main_templates', $param);
	}
	...
	public function submit($id = '')
	{
		...
		if($this->form_validation->run() == false)
		{
			...
			redirect('pengaturan_sesi/form/' . $id);
		}
		else
		{
			if(empty($id))
			{
				$proses = $this->pengaturan_sesi_model->insert($data_post);
				if($proses)
				{
			...
			{
				$proses = $this->pengaturan_sesi_model->update($data_post, $id);
				if($proses)
				{
		...
		redirect('pengaturan_sesi');
	}

	public function hapus($id)
	{
		$proses = $this->pengaturan_sesi_model->delete($id);
