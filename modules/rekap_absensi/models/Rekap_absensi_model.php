...
class Rekap_absensi_model extends CI_Model
{
	function get_data($filter = array())
	{
		...
		if(!empty($filter))
		{
			...
			{
				$this->db->where('d.kelas_id', $filter['kelas']);
			}

			if(!empty($filter['bulan']))
			{
				$this->db->where('MONTH(d.waktu)', $filter['bulan']);				
			}

			if(!empty($filter['tahun']))
			{
				$this->db->where('YEAR(d.waktu)', $filter['tahun']);								
			}

			if(!empty($filter['user_id']))
			{
				$this->db->where('d.user_id', $filter['user_id']);
			}
			...
			if(!empty($filter['opt_jenis']))
			{
				...
				{
					$query_str .= "SUM(CASE WHEN d.status = '$key' THEN 1 ELSE 0 END) AS total_$key,";
				}
				$query_str = substr($query_str, 0, -1);
			}
		}

		$this->db->order_by('a.sekolah_id ,kelas asc');
		$this->db->select("a.sekolah_id,e.kelas_id,MONTH(d.waktu) as tgl,
			a.nama,CONCAT(e.jenjang, ' ', f.nama, ' ', e.nama) AS kelas,COUNT(DISTINCT c.nis)total_siswa," . 
			$query_str
		);
		$this->db->from('profil_sekolah a');
		
		$this->db->join('user_siswa c', 'c.sekolah_id = a.sekolah_id');
                $this->db->join('user b', 'c.user_id = b.user_id');
		$this->db->join('absensi d', 'd.user_id = c.user_id');
                $this->db->join('master_kelas e', 'e.kelas_id = d.kelas_id');
		$this->db->join('master_jurusan f', 'f.jurusan_id = e.jurusan_id');	
		$this->db->group_by('c.kelas_id,a.sekolah_id');
