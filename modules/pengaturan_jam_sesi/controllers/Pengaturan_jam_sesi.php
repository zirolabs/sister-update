...
class Pengaturan_jam_sesi extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model('pengaturan_jam_sesi_model');
		$this->load->model('profil_sekolah/profil_sekolah_model');
		$this->load->model('pengaturan_sesi/pengaturan_sesi_model');

		$this->page_active 		= 'pengaturan';
		$this->sub_page_active 	= 'pengaturan_jam_sesi';
	}
	...
	public function index()
	{
		...
		$param['data']			= $this->pengaturan_jam_sesi_model->get_data($filter)->result();

		unset($filter['limit']);
		unset($filter['offset']);
		$total_rows 			= $this->pengaturan_jam_sesi_model->get_data($filter)->num_rows();
		$param['pagination']	= paging('pengaturan_jam_sesi/index', $total_rows, $limit, $uri_segment);

		$param['opt_sekolah']	= $this->profil_sekolah_model->get_opt('Semua Sekolah');

		$param['main_content']	= 'pengaturan_jam_sesi/table';
	...
	public function form($id = '')
	{
		...
		else
		{
			if(!empty($id))
			{
				$param['data'] = $this->pengaturan_jam_sesi_model->get_data_row($id);
			}
		}

		$param['opt_sekolah']	= $this->profil_sekolah_model->get_opt();
		$param['opt_sesi']		= $this->pengaturan_sesi_model->get_opt();

		$param['main_content']		= 'pengaturan_jam_sesi/form';
		$param['page_active'] 		= $this->page_active;
		$param['sub_page_active'] 	= $this->sub_page_active;
		$this->templates->load('main_templates', $param);
	}
	...
	public function submit($id = '')
	{
		...
		if($this->form_validation->run() == false)
		{
			...
			redirect('pengaturan_jam_sesi/form/' . $id);
		}
		else
		{
			if(empty($id))
			{
				$proses = $this->pengaturan_jam_sesi_model->insert($data_post);
				if($proses)
				{
			...
			{
				$proses = $this->pengaturan_jam_sesi_model->update($data_post, $id);
				if($proses)
				{
		...
		redirect('pengaturan_jam_sesi');
	}

	public function hapus($id)
	{
		$proses = $this->pengaturan_jam_sesi_model->delete($id);
